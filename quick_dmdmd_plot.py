...
end = 100  #ending iteration
axis = [0, 360, 0, 360] #specify the axis you desire
smoothing = 1 #0-inf, increase to increase amount of smoothing. 0 is no smoothing.
phi_shift = 0
chi_shift = 120
...
weights = np.loadtxt("iter%d.w" % start)
values = np.loadtxt("iter%d-dihedral.xvg" % start, skiprows=29, usecols=(0,1))
phi = np.array(values[:,0])
chi = np.array(values[:,1])
for i in np.arange(start+1, end+1, 1):
    w = np.loadtxt("iter%d.w" % i)
    v = np.loadtxt("iter%d-dihedral.xvg" % i, skiprows=29, usecols=(0,1))
    weights = np.append(weights,w)
    phi = np.append(phi, v[:,0])
    chi = np.append(chi, v[:,1])


plt.figure()
plot_style = "scatter"
x, y, z = make(phi, chi, 50, 50, temperature=300, weight=weights, plot_style="scatter", free_energy_plot=True, idx_smoothing=smoothing)
cp = plt.scatter((x+360+phi_shift)%360, (y+360+chi_shift)%360, s=10, c=z, marker='o', linewidth=0.)

if not axis == None:
    plt.axis(axis)
plt.xlabel("phi (deg.)", size=16)
plt.ylabel("chi (deg.)", size=16)
